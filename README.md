...
* ```javascript
  var prettified = oh.prettifyValue(conf, rule_index);
  ```
  ...
  The function excepts two optional argument. The first is expected to be a hash. This hash can hold configuration options. One example: ```{ block_sep_string: '\n', print_semicolon: false }```. Look in the source code if you need more.

  The second parameter can be used to only pretiffy a specific rule. rule_index is the internal index of the rule as returned by oh.getMatchingRule(date).
